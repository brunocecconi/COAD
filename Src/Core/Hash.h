...
// FNV-1a 64 bit hash
static constexpr fnv1a_t Fnv1AHash(const Size n, const char * str, const fnv1a_t hash = FNV_BASIS)
{
    return n > 0 ? Fnv1AHash(n - 1, str + 1, (hash ^ *str) * FNV_PRIME) : hash;
}

template<Size N>
static constexpr fnv1a_t Fnv1AHash(const char (&array)[N])
{
...
{
    return Hash::Fnv1AHash(length, str);
}
